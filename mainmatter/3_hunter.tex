...
\hugo{Añadir código y datasets en los resúmenes.}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/hunter_flow_with_map.png}
...
where $d$ represents the maximum distance between the probe and the destination, $RTT$ corresponds to the measured round-trip time, $c$ denotes the speed of light in a vacuum, and $n$ is the refractive index of the optic fibre, 1.468.
...
The previous formulation assumes that packets travel through geographically optimal paths at the maximum propagation speed supported by optical fibre. However, real Internet routing paths rarely satisfy these conditions. Routing policies, peering agreements, traffic engineering mechanisms, and physical infrastructure constraints often introduce significant path inflation, causing packets to traverse substantially longer distances than the direct geographic path between the source and destination.
...
The methodology proposed in this chapter relies on active Internet measurements, routing-path analysis, and latency-based geographic inference to estimate the destination jurisdictions associated with anycast communications. While the obtained validation results demonstrate that Hunter provides reliable country-level inference under real-world conditions, the methodology is inherently affected by several limitations associated with Internet-scale measurements and operational routing infrastructures.
...
First of all, traceroute can be used with UDP, ICMP, or TCP protocols, producing different behaviours depending on the protocol selected. The default and traditional protocol used is UDP. Packets sent with UDP are directed to a port, in the Linux implementation 33434, that has no usage in the common network services deployed on the Internet. This type of packet could be filtered by firewalls, producing a path limited to the network element prior to the firewall. If ICMP packets are selected, then standard ICMP echo packets are used, the same as those used for ping. These packets could have an advantage over UDP since ICMP packets are usually not filtered by network elements, as they are used by the owners of these elements for diagnosis. However, some very restrictive organisations do not reply to this protocol, or the ICMP packets are discarded. Finally, when TCP packets are selected, they are sent to the destination port 80, the same one used for HTTP. Since the HTTP port is rarely filtered, these TCP packets are not filtered either. Although TCP could be the most convenient protocol, this is not true in all cases. Since the TCP option is used, the packets sent are treated as HTTP. Therefore, the routing could be different, with fewer intermediate network elements or network elements behaving differently in various ways because of a quality of service policy, resulting in hidden network elements.

This execution of traceroute with the three different protocols used under the same conditions shows the differences:\hugo{This example is here to show the behaviour it but I do not know what format to use.}

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/hunter_traceroute_protocols_examples.png}
...
The validation experiments presented in the previous section illustrate the practical impact of these routing dynamics, particularly in Northern European countries such as Finland and Norway, where substantial variability in destination attribution was observed across repeated measurements.

Importantly, these situations do not necessarily indicate incorrect operation of the methodology but rather reflect the intrinsic instability of the underlying anycast routing system. In practice, the geographic destination of an anycast communication may genuinely change over time depending on the operational state of the network.
...
First, the methodology depends partially on commercial IP geolocation databases to estimate the location of the last hop router. Although router infrastructure is generally more stable than end-user or cloud-server addresses, geolocation databases may still contain outdated or inaccurate information. Inaccurate last hop geolocation may affect the selection of nearby vantage points and increase the uncertainty of the resulting multilateration region.

Second, Hunter approximates destination metropolitan areas using airport locations inspired by previous work \cite{Cicalese2016Latency-BasedSets}. While this approach provides practical and scalable geographic references, airports do not necessarily correspond exactly to the physical location of the serving infrastructure. Large metropolitan regions may contain multiple datacenters distributed across tens of kilometres, introducing additional uncertainty into city-level inference.

Furthermore, certain multilateration regions may overlap multiple metropolitan areas or cross national borders. This situation is particularly relevant in densely interconnected regions, where neighbouring countries may be separated by relatively small geographic distances. Under these circumstances, small variations in latency measurements or routing behaviour may produce ambiguity regarding the final jurisdiction associated with the communication.
...
These limitations highlight the inherent uncertainty associated with Internet-scale geographic inference. Nevertheless, the validation results demonstrate that, despite these constraints, Hunter provides sufficiently reliable country-level attribution capabilities to support the large-scale analysis of international data transfers over anycast infrastructures.
...
Since one of the main goals of this thesis is to analyse compliance with GDPR requirements regarding international transfers of personal data, the validation was intentionally designed around European jurisdictions rather than as a globally exhaustive evaluation. The experiments were therefore conducted from 29 countries belonging to the European Economic Area (EEA). To emulate communications originating from different European jurisdictions, a commercial VPN provider was used to establish exit nodes in all except one EEA member state. Each validation experiment was repeated from multiple European countries to evaluate the behaviour of Hunter under different routing conditions and geographic origins. Liechtenstein could not be included in the validation because the selected VPN provider did not provide an exit node in that country.

One important limitation of the validation methodology is the use of a VPN service to emulate communications originating from different EEA countries instead of directly using RIPE Atlas probes as measurement origins. Ideally, the traceroute and latency measurements performed by Hunter should be executed directly from distributed RIPE Atlas probes while simultaneously obtaining the HTTP headers returned by the target service in order to guarantee that both observations correspond to the exact same communication context and anycast replica.
...
Consequently, the validation required executing both the HTTP request and Hunter from the same controlled environment, which was achieved through the use of VPN exit nodes located in different EEA countries. Although this approach may introduce slight differences with respect to native residential or RIPE Atlas routing conditions, it guarantees that the ground-truth HTTP request and the Hunter measurements observe the same anycast routing behaviour at the time of execution.

\hugo{Maybe is counter productive that it was focused in european territory. Include additional details regarding the VPN provider and measurement repetition strategy. I need to evaluate the option of making a new and more reproducible and rigorous validation of Hunter.}
...
In addition to evaluating the complete Hunter methodology, the validation process also compares Hunter against a simpler baseline approach consisting of directly geolocating the last hop IP address using the IPinfo service. This comparison allows for assessing the benefits introduced by multilateration and conservative inference mechanisms beyond straightforward IP geolocation techniques.

\subsection{Results}

\hugo{Include total number of experiments executed during the validation process.}

The obtained results indicate that Hunter provides highly reliable country-level inference for anycast communications under real-world Internet conditions. Across all evaluated scenarios, Hunter achieves a precision of 97.67\% and an accuracy of 78.09\%.

\hugo{Include the number of Positive, Negative and Indeterminate results, number of measurements and number of countries.}
...
These results indicate that Hunter successfully minimises incorrect country attributions while still identifying the geographic destination of a substantial proportion of communications. The difference between precision and accuracy reflects the conservative nature of the methodology, which intentionally prioritises soundness over completeness by returning \textit{Indeterminate} results whenever sufficient confidence cannot be achieved.

The comparison against the baseline last hop geolocation approach further highlights the benefits of the proposed methodology. Directly geolocating the last hop IP address using the IPinfo service slightly increases the number of identified destinations (+0.44\%), but also produces a substantially larger number of incorrect attributions (+13.72\%).

This result demonstrates that simple last hop geolocation techniques may improve completeness at the cost of significantly reducing reliability. In the context of privacy analysis and legal compliance assessment, incorrectly assigning communications to a jurisdiction may lead to erroneous conclusions regarding international data transfers. Consequently, Hunter deliberately favours conservative inference decisions that minimise false positives, even if certain transfers remain unresolved.

A detailed analysis of the Negative results reveals that 94.12\% of all incorrect attributions (48 out of 51) are concentrated in Norway and Finland, where Hunter achieves accuracies of 50.72\% and 80.28\%, respectively. These observations suggest that the routing behaviour associated with Northern European networks introduces substantial instability in the geographic association between probes and anycast replicas.

\hugo{Clarify this effect of 100\% accuracy and respect to what effect is considered. Include more detail about the exact number of repetitions. Maybe a figure table summarizing Positive, Negative, and Indeterminate distributions by country.}

Additional experiments further showed that repeatedly executing measurements over time significantly mitigates these inconsistencies. Under repeated measurements and after filtering statistically unstable destinations, no incorrect attributions were observed in the evaluated scenarios. Empirical observations indicate that destinations appearing in more than 5\% of repeated executions can be considered statistically stable and therefore reliable for country-level attribution.

\subsection{Cross-border Communications during Validation}

The communications observed during the validation process already reveal a significant number of international data transfers associated with anycast infrastructures. Figure~\ref{fig:hunter_validation_flows} illustrates the communication flows obtained during the validation experiments targeting the IP address 192.5.5.241. Flows crossing the borders of the European Economic Area are highlighted in red.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figures/hunter_validation_flows.png}
...
The obtained validation results confirm that Hunter provides sufficiently reliable geographic inference capabilities for studying international data transfers over anycast infrastructures. Building upon these results, the following chapters employ Hunter to analyse real-world mobile ecosystems and evaluate the impact of anycast communications on data protection and jurisdictional exposure.

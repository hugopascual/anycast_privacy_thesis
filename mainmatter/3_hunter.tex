...
Formally, the maximum distance associated with a probe can be estimated as:

\hugo{Add the mathematical expression}
:contentReference[oaicite:0]{index=0}

where $d$ represents the maximum distance between the probe and the destination, $RTT$ corresponds to the measured round-trip time, and $v$ denotes the assumed propagation speed of packets through the network medium.
...
\hugo{Include here the Verloc approximation in order to get a more realistic distance travelled.}

To reduce the impact of transient congestion and network variability, Hunter considers the minimum RTT observed across repeated measurements. This approach minimizes the influence of temporary queuing delays and provides a more stable approximation of the propagation component of the communication latency.
...
The geographic regions obtained during the multilateration phase may vary substantially in size depending on probe distribution, routing behavior, and latency consistency. Consequently, an additional inference step is required to transform the estimated geographic area into a country-level attribution.

The proposed methodology is based on the observation that large-scale anycast infrastructures are typically deployed near major metropolitan and interconnection areas. This behavior is common among Content Delivery Networks (CDNs), cloud providers, and distributed Internet services, whose infrastructures are frequently located close to Internet Exchange Points (IXPs) and major population centers. Inspired by previous work in the field \cite{Cicalese2016Latency-BasedSets}, Hunter therefore uses major airports as geographic references approximating the metropolitan regions where anycast replicas are likely deployed.
